...
const { ethers, networkHelpers, networkName, networkConfig, provider } = await hre.network.connect()
...
!developmentChains.includes(networkName)
  ...
  : describe("Raffle Unit Tests", async function () {
      ...
      async function deployVRFMockFixture() {
        ...
        const deployContract = await ethers.deployContract("Raffle", raffleArgs)
        await deployContract.waitForDeployment()

        // Ensure the Raffle contract is a valid consumer of the VRFCoordinatorV2Mock contract.
        if (developmentChains.includes(networkName)) {
          // addConsumer
          await vrfCoordinatorV2_5Mock.addConsumer(subscriptionId, deployContract.target)
        }
        // 连接合约
      ...
      describe("constructor", function () {
        it("initializes the raffle correctly", async () => {
          ...
          const { raffle, chainId, interval } = await networkHelpers.loadFixture(deployVRFMockFixture)
          const raffleState = (await raffle.getRaffleState()).toString()
          // Comparisons for Raffle initialization:
          assert.equal(raffleState, "0")
          assert.equal(interval.toString(), networkConfigs[chainId!]["keepersUpdateInterval"])
        })
      })
      ...
      describe("enterRaffle", async function () {
        it("reverts when you don't pay enough", async () => {
          ...
          const { raffle } = await networkHelpers.loadFixture(deployVRFMockFixture)
          await expect(raffle.enterRaffle()).to.be.rejectedWith("Raffle__SendMoreToEnterRaffle")
        })
        it("records player when they enter", async () => {
          const { raffleEntranceFee } = await networkHelpers.loadFixture(deployVRFMockFixture)
        ...
        it("doesn't allow entrance when raffle is calculating", async () => {
          const { raffle, raffleEntranceFee, interval, vrfCoordinatorV2_5Mock } =
          ...
          await expect(raffle.enterRaffle({ value: raffleEntranceFee })).to.be.rejectedWith("Raffle__RaffleNotOpen()")
        })
      })

      describe("checkUpkeep", function () {
        it("returns false if people haven't sent any ETH", async () => {
          const { raffle, interval } = await networkHelpers.loadFixture(deployVRFMockFixture)
          await ethers.provider.send("evm_increaseTime", [Number(interval) + 1])
          await ethers.provider.send("hardhat_mine", ["0x1"])
        ...
        it("returns false if raffle isn't open", async () => {
          const { raffle, raffleEntranceFee, interval } = await networkHelpers.loadFixture(deployVRFMockFixture)
          await raffle.enterRaffle({ value: raffleEntranceFee })
          await ethers.provider.send("evm_increaseTime", [Number(interval) + 1])
          await ethers.provider.send("hardhat_mine", ["0x1"])
        ...
        it("returns false if enough time hasn't passed", async () => {
          const { raffle, raffleEntranceFee, interval } = await networkHelpers.loadFixture(deployVRFMockFixture)
          await raffle.enterRaffle({ value: raffleEntranceFee })
          await ethers.provider.send("evm_increaseTime", [Number(interval) - 5])
        ...
        it("returns true if enough time has passed, has players, eth, and is open", async () => {
          ...
          const timePassed = (await ethers.provider.getBlock("latest"))!.timestamp - Number(currentTimestamp)
          const playerCount = await raffle.getNumberOfPlayers()
          const contractBalance = await ethers.provider.getBalance(raffle.target)

          console.log(`Raffle state: ${raffleState}, Time passed: ${timePassed}, Required interval: ${timeInterval}`)
          console.log(`Player count: ${playerCount}, Contract balance: ${contractBalance}`)
      ...
      describe("performUpkeep", function () {
        ...
        it("reverts if checkup is false", async () => {
          const { raffle } = await networkHelpers.loadFixture(deployVRFMockFixture)
          await expect(raffle.performUpkeep("0x")).to.be.rejectedWith("Raffle__UpkeepNotNeeded")
        })
        it("updates the raffle state and emits a requestId", async () => {
          ...
          const requestIdLog = txReceipt?.logs
            ...
            .find((event: any) => event?.name === "RequestedRaffleWinner")
      ...
      describe("fulfillRandomWords", function () {
        ...
        it("can only be called after performupkeep", async () => {
          const { raffle, vrfCoordinatorV2_5Mock } = await networkHelpers.loadFixture(deployVRFMockFixture)
        ...
        it("picks a winner, resets, and sends money", async () => {
          const { raffle, raffleEntranceFee, vrfCoordinatorV2_5Mock, deployContract } =
            await networkHelpers.loadFixture(deployVRFMockFixture)
          ...
          for (let i = startingIndex; i < startingIndex + additionalEntrances; i++) {
            ...
            console.log(`account :${i} ,address:${accounts[i].address}`)
            const tmpRaffleContract: any = deployContract.connect(accounts[i])
          ...
          // This will be more important for our staging tests...事件监听
          await new Promise<void>(async (resolve, reject) => {
            ...
            raffle.once("WinnerPicked", async () => {
              ...
              try {
                ...
                const winnerBalance = await ethers.provider.getBalance(accounts[4].address)
                //0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65 0n 9999989960014490104422n 1768616822n
                const account4Balance = await ethers.provider.getBalance(accounts[4].address)
                console.log(account4Balance)

                assert.equal(recentWinner.toString(), accounts[4].address)
                assert.equal(raffleState, 0)
                // 优胜者最后的余额应该等同于所有人在合约中投入的金额： 他的初始余额 + 所有参赛者的入场费之和
                assert.equal(
                  winnerBalance.toString(),
                  (BigInt(startingBalance) + BigInt(raffleEntranceFee) * BigInt(additionalEntrances)).toString(),
                )
                console.log("XXX")
            ...
            try {
              await ethers.provider.send("evm_increaseTime", [Number(await raffle.getInterval()) + 1])
              await ethers.provider.send("hardhat_mine", ["0x1"])
              // 模拟链环节守护进程调用 performUpkeep 来触发随机数请求
              const tx = await raffle.performUpkeep("0x")
              const txReceipt = await tx.wait(1)

              startingBalance = await ethers.provider.getBalance(accounts[4].address)

              console.log(txReceipt)
              console.log(txReceipt.logs)
              console.log(raffle.target)
              console.log(vrfCoordinatorV2_5Mock.target)
              console.log("YYY")
              // ethers.js v6: 从 logs 中解析事件
              const requestIdLog = txReceipt?.logs
                ?.map((log: any) => {
